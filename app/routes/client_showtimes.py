...
def get_showtimes_by_movie(movie_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        logger.error("Error en GET /client/showtimes/movie/%s: %s", movie_id, exc, exc_info=True)
...
def get_showtimes_by_date(
    ...
    try:
        ...
        logger.error("Error en GET /client/showtimes/date/%s: %s", target_date, exc, exc_info=True)
        raise HTTPException(status_code=500, detail=str(exc))