...
router = APIRouter(prefix="/client/seats", tags=["client seats"])

@router.get("/showtime/{showtime_id}", response_model=SeatMapResponse)
def get_seat_map(showtime_id: int, db: Session = Depends(get_db)):
    logger.info("GET /client/seats/showtime/%s", showtime_id)
    try:
        return get_showtime_seat_map(db, showtime_id)
    except HTTPException:
        raise
    except Exception as exc:
        logger.error("Error en GET /client/seats/showtime/%s: %s", showtime_id, exc, exc_info=True)
...
def lock_seats(payload: SeatLockRequest, db: Session = Depends(get_db)):
    logger.info("POST /client/seats/lock - funcion=%s asientos=%s", payload.id_funcion, payload.ids_asientos)
    try:
        ...
        logger.error("Error en POST /client/seats/lock: %s", exc, exc_info=True)
        raise HTTPException(status_code=500, detail=str(exc))